...
namespace agg
{
    ...
    {
        m_subpixel_width = uround(w * (double)subpixel_scale);
    ...
    {
        double base_val = 1.0;
        if(center_width == 0.0)   center_width = 1.0 / (double)subpixel_scale;
        if(smoother_width == 0.0) smoother_width = 1.0 / (double)subpixel_scale;
        ...
        unsigned subpixel_center_width = unsigned(center_width * (double)subpixel_scale);
        unsigned subpixel_smoother_width = unsigned(smoother_width * (double)subpixel_scale);
        ...
        unsigned val = m_gamma[unsigned(base_val * (double)aa_mask)];
        ch = ch_center;
        ...
        for(i = 0; i < subpixel_smoother_width; i++)
        {
            *ch_smoother++ = 
                m_gamma[unsigned((base_val - 
                                  base_val * 
                                  (double(i) / subpixel_smoother_width)) * (double)aa_mask)];
        }
