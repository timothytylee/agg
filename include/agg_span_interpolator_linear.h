...
namespace agg
{
    ...
    public:
        ...
        void begin(double x, double y, unsigned len)
        {
            ...
            m_li_x = dda2_line_interpolator(x1, iround(tx * (double)subpixel_scale), len);
            m_li_y = dda2_line_interpolator(y1, iround(ty * (double)subpixel_scale), len);
        }
        ...
        void operator++()
        {
            ...
            if(m_pos >= m_subdiv_size)
            {
                ...
                double ty = m_src_y;
                m_trans->transform(&tx, &ty);
                m_li_x = dda2_line_interpolator(m_li_x.y(), iround(tx * (double)subpixel_scale), len);
                m_li_y = dda2_line_interpolator(m_li_y.y(), iround(ty * (double)subpixel_scale), len);
                m_pos = 0;
            }
